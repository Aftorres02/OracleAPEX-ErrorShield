create or replace package body ersh_logger
as
  ...
  --  Primarily used in the trigger on ersh_logger_logs.
  --
  -- $$APEX
  --  Set in ersh_logger_configure.
  ...
  -- #46
  g_plug_logger_log_error rec_ersh_logger_log;
  ...
  gc_pref_level constant ersh_logger_prefs.pref_name%type := 'LEVEL';
  gc_pref_include_call_stack constant ersh_logger_prefs.pref_name%type := 'INCLUDE_CALL_STACK';
  gc_pref_protect_admin_procs constant ersh_logger_prefs.pref_name%type := 'PROTECT_ADMIN_PROCS';
  gc_pref_install_schema constant ersh_logger_prefs.pref_name%type := 'INSTALL_SCHEMA';
  gc_pref_purge_after_days constant ersh_logger_prefs.pref_name%type := 'PURGE_AFTER_DAYS';
  gc_pref_purge_min_level constant ersh_logger_prefs.pref_name%type := 'PURGE_MIN_LEVEL';
  gc_pref_logger_version constant ersh_logger_prefs.pref_name%type := 'LOGGER_VERSION';
  gc_pref_client_id_expire_hours constant ersh_logger_prefs.pref_name%type := 'PREF_BY_CLIENT_ID_EXPIRE_HOURS';
  gc_pref_logger_debug constant ersh_logger_prefs.pref_name%type := 'LOGGER_DEBUG';
  gc_pref_plugin_fn_error constant ersh_logger_prefs.pref_name%type := 'PLUGIN_FN_ERROR';
   ...
   */
  function get_param_clob(p_params in ersh_logger.tab_param)
    return clob
  as
    l_return clob;
  ...
  function set_extra_with_params(
    p_extra in ersh_logger_logs.extra%type,
    p_params in tab_param
  )
    return ersh_logger_logs.extra%type
  as
    l_extra ersh_logger_logs.extra%type;
  begin
    $if $$no_op $then
      return null;
    $else
 ...
 function admin_security_check
    return boolean
  is
    l_protect_admin_procs ersh_logger_prefs.pref_value%type;
    l_return boolean default false;
  begin
    $if $$no_op $then
      l_return := true;
    $else
      l_protect_admin_procs := get_pref(ersh_logger.gc_pref_protect_admin_procs);
      if l_protect_admin_procs = 'TRUE' then
        if get_pref(ersh_logger.gc_pref_install_schema) = sys_context('USERENV','SESSION_USER') then
          l_return := true;
        else
          l_return := false;
  ...
  function get_level_number
    ...
    $if $$rac_lt_11_2 $then
      $if not dbms_db_version.ver_le_10_2 $then
        result_cache relies_on (ersh_logger_prefs, ersh_logger_prefs_by_client_id)
      $end
    $end
  is
    l_level number;
  ...
  begin
    ...
    $else
      ...
      l_level := convert_level_char_to_num(ersh_logger.get_pref(ersh_logger.gc_pref_level));
  ...
  function include_call_stack
    ...
    $end
  is
    l_call_stack_pref ersh_logger_prefs.pref_value%type;
  begin
    $if $$no_op $then
      return false;
    $else
      $if $$rac_lt_11_2 $then
        l_call_stack_pref := get_pref(ersh_logger.gc_pref_include_call_stack);
      $else
  ...
  /**
   * Main procedure that will store log data into ersh_logger_logs table
   *
   *
   * Modifications
  ...
  procedure log_internal(
    ...
    p_params in tab_param default ersh_logger.gc_empty_tab_param)
  is
  ...
  begin
    ...
    $else
      ...
      if p_callstack is not null and ersh_logger.include_call_stack then
        ersh_logger.get_debug_info(
   ...
   */
  procedure run_plugin(p_logger_log in ersh_logger.rec_ersh_logger_log)
  as
    l_plugin_fn ersh_logger_prefs.pref_value%type;
    ...
    l_scope ersh_logger_logs.scope%type;
    ...
    as
    begin
      if p_logger_log.logger_level = ersh_logger.g_error then
  ...
  begin
    ...
    $if $$ersh_logger_debug $then
      dbms_output.put_line('in run_plugin. g_in_plugin_error: ' || ersh_logger.tochar(g_in_plugin_error));
    $end

    if 1=2 then
      null;
    elsif p_logger_log.logger_level = ersh_logger.g_error then
    ...
    if l_plugin_ctx is not null then
      ...
      if 1=1
        ...
        l_sql := 'begin ' || l_plugin_fn || '(ersh_logger.get_plugin_rec(' || p_logger_log.logger_level || ')); end;';
        ...
        ersh_logger.log_error('Error l_plugin_fn does not have value');
      end if; -- l_plugin_fn
    else
      -- Should never reach this point since plugin_ctx should have a value
      ersh_logger.log_error('Error l_plugin_ctx does not have value');
  ...
  exception
    when others then
      ersh_logger.append_param(l_params, 'Logger.id', p_logger_log.id);
      ersh_logger.append_param(l_params, 'Logger.logger_level', p_logger_log.logger_level);
      ersh_logger.append_param(l_params, 'Plugin Function', l_plugin_fn);
      ...
      ersh_logger.log_error('Exception in plugin procedure: ' || l_plugin_fn, l_scope, null, l_params);
   ...
   *
   * Notes:
   *  - Though this is public it is not a documented procedure. Only used with ersh_logger_configure.
   *
   * Related Tickets:
   *  - #46 Plugin support
  ...
  /**
   * Store APEX items in ersh_logger_logs_apex_items
   *
   * Notes:
   *  -
   *
   ...
   * @author Tyler Muth
   * @created ???
   * @param p_log_id ersh_logger_logs.id to reference
  ...
  procedure snapshot_apex_items(
    p_log_id in ersh_logger_logs.id%type,
  ...
  begin
    ...
    $else
      $if $$apex $then
        ...
        if ersh_logger.is_number(l_item_type) then
          l_item_type_page_id := to_number(l_item_type);
        end if;

        insert into ersh_logger_logs_apex_items(log_id,app_session,item_name,item_value)
        ...
        from (
          ...
          where 1=1
            ...
            and l_item_type in (ersh_logger.g_apex_item_type_all, ersh_logger.g_apex_item_type_app)
          union all
          ...
          where 1=1
            ...
            and (
              1=2
              or l_item_type in (ersh_logger.g_apex_item_type_all, ersh_logger.g_apex_item_type_page)
  ...
  begin
    $if $$no_op $then
      null;
    $else
      if ok_to_log(ersh_logger.g_error) then
        get_debug_info(
        ...
        ins_ersh_logger_logs(
          p_unit_name => upper(l_proc_name) ,
          p_scope => p_scope ,
          p_logger_level => ersh_logger.g_error,
          p_extra => l_extra,
          p_text => l_text,
        ...
        -- Plugin
        $if $$ersh_logger_plugin_error $then

          if not g_in_plugin_error then
            g_plug_logger_log_error.logger_level := ersh_logger.g_error;
  ...
  begin
    $if $$no_op $then
      null;
    $else
      if ok_to_log(ersh_logger.g_permanent) then
        log_internal(
          p_text => p_text,
          p_log_level => ersh_logger.g_permanent,
          p_scope => p_scope,
          p_extra => p_extra,
          p_callstack => dbms_utility.format_call_stack,
          p_params => p_params
  ...
  begin
    $if $$no_op $then
      null;
    $else
      if ok_to_log(ersh_logger.g_warning) then
        log_internal(
          p_text => p_text,
          p_log_level => ersh_logger.g_warning,
          p_scope => p_scope,
          p_extra => p_extra,
          p_callstack => dbms_utility.format_call_stack,
          p_params => p_params);
  ...
  begin
    $if $$no_op $then
      null;
    $else
      if ok_to_log(ersh_logger.g_information) then
        log_internal(
          p_text => p_text,
          p_log_level => ersh_logger.g_information,
          p_scope => p_scope,
          p_extra => p_extra,
          p_callstack => dbms_utility.format_call_stack,
          p_params => p_params);
  ...
  begin
    ...
    $if $$no_op $then
      null;
    $else
      if ok_to_log(ersh_logger.g_debug) then
        log_internal(
          p_text => p_text,
          p_log_level => ersh_logger.g_debug,
          p_scope => p_scope,
          p_extra => p_extra,
          p_callstack => dbms_utility.format_call_stack,
          p_params => p_params);
   ...
   * @param p_show_null
   * @param p_scope
   * @param p_level Highest level to run at (default ersh_logger.g_debug). Example. If you set to ersh_logger.g_error it will work when both in DEBUG and ERROR modes. However if set to ersh_logger.g_debug(default) will not store values when level is set to ERROR.
   */
  procedure log_userenv(
    p_detail_level in varchar2 default 'USER',-- ALL, NLS, USER, INSTANCE,
    p_show_null in boolean default false,
    p_scope in ersh_logger_logs.scope%type default null,
    p_level in ersh_logger_logs.logger_level%type default null)
  is
    l_extra clob;
  begin
    $if $$no_op $then
      null;
    $else
      if ok_to_log(nvl(p_level, ersh_logger.g_debug)) then
        ...
        log_internal(
          ...
          p_log_level => nvl(p_level, ersh_logger.g_sys_context),
          p_scope => p_scope,
          p_extra => l_extra);
      end if;
    $end
    ...
    $else
      if ok_to_log(nvl(p_level, ersh_logger.g_debug)) then
        ...
        log_internal(
          ...
          p_log_level => nvl(p_level, ersh_logger.g_debug),
          p_scope => p_scope,
          p_extra => l_dump);
      end if;
    $end
  ...
  procedure log_apex_items(
    ...
    p_scope in ersh_logger_logs.scope%type default null,
    p_item_type in varchar2 default ersh_logger.g_apex_item_type_all,
    ...
    $else
      if ok_to_log(nvl(p_level, ersh_logger.g_debug)) then
        ...
        $if $$apex $then
          ...
          assert(
            p_condition => upper(p_item_type) in (ersh_logger.g_apex_item_type_all, ersh_logger.g_apex_item_type_app, ersh_logger.g_apex_item_type_page) or ersh_logger.is_number(p_item_type),
            p_message => ersh_logger.sprintf('APEX Item Scope was set to %s. Must be %s, %s, %s, or page number', p_item_type, ersh_logger.g_apex_item_type_all, ersh_logger.g_apex_item_type_page, ersh_logger.g_apex_item_type_page));

          log_internal(
            p_text => p_text,
            p_log_level => nvl(p_level, ersh_logger.g_apex),
            p_scope => p_scope);
        ...
        $else
          ...
          log_internal(
            p_text => l_error,
            p_log_level => ersh_logger.g_apex,
            p_scope => p_scope);
        $end
      end if;
    $end
    ...
    $else
      if ok_to_log(ersh_logger.g_debug) then
        ...
        if g_running_timers > 1 then
          ...
          l_pad := replace(lpad('a',ersh_logger.g_running_timers,'>')||' ', 'a', null);
        end if;
   ...
   *
   * Related Tickets:
   *  - #127: Added ersh_logger_prefs.pref_type
   *
   * @author Tyler Muth
   * @created ???
   *
   ...
   */
  function get_pref(
    p_pref_name in ersh_logger_prefs.pref_name%type,
    p_pref_type in ersh_logger_prefs.pref_type%type default ersh_logger.g_pref_type_logger)
    return varchar2
    $if not dbms_db_version.ver_le_10_2  $then
      result_cache
      $if $$no_op is null or not $$no_op $then
        relies_on (ersh_logger_prefs, ersh_logger_prefs_by_client_id)
      $end
    $end
  is
    l_scope varchar2(30) := 'get_pref';
    l_pref_value ersh_logger_prefs.pref_value%type;
    l_client_id ersh_logger_prefs_by_client_id.client_id%type;
    l_pref_name ersh_logger_prefs.pref_name%type := upper(p_pref_name);
    l_pref_type ersh_logger_prefs.pref_type%type := upper(p_pref_type);
  begin
    ...
    $else
      ...
      from (
        ...
        from (
          ...
          select
            case
              when l_pref_name = ersh_logger.gc_pref_level then logger_level
              when l_pref_name = ersh_logger.gc_pref_include_call_stack then include_call_stack
            end pref_value,
            1 rank
          from ersh_logger_prefs_by_client_id
          where 1=1
            ...
            and l_pref_type = ersh_logger.g_pref_type_logger
            and l_pref_name in (ersh_logger.gc_pref_level, ersh_logger.gc_pref_include_call_stack)
          union
   ...
   */
  procedure set_pref(
    p_pref_type in ersh_logger_prefs.pref_type%type,
    p_pref_name in ersh_logger_prefs.pref_name%type,
    p_pref_value in ersh_logger_prefs.pref_value%type)
  as
    l_pref_type ersh_logger_prefs.pref_type%type := trim(upper(p_pref_type));
    l_pref_name ersh_logger_prefs.pref_name%type := trim(upper(p_pref_name));
  begin

    $if $$no_op $then
      null;
    $else
      if l_pref_type = ersh_logger.g_pref_type_logger then
  ...
  is
    $if $$no_op is null or not $$no_op $then
      l_purge_after_days number := nvl(p_purge_after_days,get_pref(ersh_logger.gc_pref_purge_after_days));
    $end
    pragma autonomous_transaction;
  begin
    $if $$no_op $then
  ...
  begin
    ...
    $else
      purge(
        ...
        p_purge_min_level => convert_level_char_to_num(nvl(p_purge_min_level,get_pref(ersh_logger.gc_pref_purge_min_level))));
    $end
  end purge;
  ...
  begin
    ...
    $else
      if admin_security_check then
        delete from ersh_logger_logs where logger_level > l_purge_level;
      end if;
    $end
    commit;
  end purge_all;
  ...
  is
    ...
    l_client_identifier ersh_logger_prefs_by_client_id.client_id%type;

    -- For current client info
    l_cur_logger_level ersh_logger_prefs_by_client_id.logger_level%type;
    l_cur_include_call_stack ersh_logger_prefs_by_client_id.include_call_stack%type;
    l_cur_expiry_date ersh_logger_prefs_by_client_id.expiry_date%type;
  ...
  begin
    ...
    $else
      ...
      where 1=1
        ...
        and pref_name = ersh_logger.gc_pref_level;

      $if $$flashback_enabled $then
        l_flashback := 'Enabled';
      $end

      l_version := get_pref(ersh_logger.gc_pref_logger_version);

      display_output('Logger Version',l_version);
      display_output('Debug Level',l_debug);
      display_output('Capture Call Stack',get_pref(ersh_logger.gc_pref_include_call_stack));
      display_output('Protect Admin Procedures',get_pref(ersh_logger.gc_pref_protect_admin_procs));
      display_output('APEX Tracing',l_apex);
      display_output('SCN Capture',l_flashback);
      display_output('Min. Purge Level',get_pref(ersh_logger.gc_pref_purge_min_level));
      display_output('Purge Older Than',get_pref(ersh_logger.gc_pref_purge_after_days)||' days');
      display_output('Pref by client_id expire hours',get_pref(ersh_logger.gc_pref_client_id_expire_hours)||' hours');
      $if $$rac_lt_11_2  $then
      ...
      $if $$ersh_logger_plugin_error $then
        display_output('PLUGIN_FN_ERROR',get_pref(ersh_logger.gc_pref_plugin_fn_error));
      $end

      -- #64
      $if $$ersh_logger_debug $then
        display_output('LOGGER_DEBUG',get_pref(ersh_logger.gc_pref_logger_debug) || '   *** SHOULD BE TURNED OFF!!! SET TO FALSE ***');
      $end
      ...
      if l_client_identifier is not null then
        ...
        begin
          ...
          display_output('Client - Expiry Date', ersh_logger.tochar(l_cur_expiry_date));
        exception
          when no_data_found then
      ...
      display_output('For all client info see', 'ersh_logger_prefs_by_client_id');
   ...
   * @param p_include_call_stack Optional: Only valid if p_client_id is defined Valid values: TRUE, FALSE. If not set will use the default system pref in ersh_logger_prefs.
   ...
   */
  procedure set_level(
    p_level in varchar2 default ersh_logger.g_debug_name,
    p_client_id in varchar2 default null,
  ...
  is
    ...
    l_expiry_date ersh_logger_prefs_by_client_id.expiry_date%type;

    l_id ersh_logger_logs.id%type;
    pragma autonomous_transaction;
  begin
    $if $$no_op $then
    ...
    $else
      ...
      l_include_call_stack := nvl(trim(upper(p_include_call_stack)), get_pref(ersh_logger.gc_pref_include_call_stack));
      ...
      if p_client_id is not null or admin_security_check then
        ...
        if l_level in (ersh_logger.g_apex_name, ersh_logger.g_sys_context_name, ersh_logger.g_timing_name)  then
          ersh_logger.ins_ersh_logger_logs(
            p_logger_level => ersh_logger.g_warning,
            p_text =>
              ersh_logger.sprintf('Logger level: %s1 is deprecated. Set for client_id %s2. Automatically setting to %s3', l_level, nvl(p_client_id, '<global>'), ersh_logger.g_debug_name),
            po_id => l_id);

          l_level := ersh_logger.g_debug_name;
        end if;


        -- Separate updates/inserts for client_id or global settings
        if p_client_id is not null then
          l_client_id_expire_hours := nvl(p_client_id_expire_hours, get_pref(ersh_logger.gc_pref_client_id_expire_hours));
          l_expiry_date := sysdate + l_client_id_expire_hours/24;

          merge into ersh_logger_prefs_by_client_id ci
        ...
        ersh_logger.save_global_context(
          p_attribute => gc_ctx_attr_level,
          p_value => ersh_logger.convert_level_char_to_num(l_level),
        ...
        ersh_logger.ins_ersh_logger_logs(
          p_logger_level => ersh_logger.g_information,
  ...
  begin
    ...
    $else
      ...
      delete from ersh_logger_prefs_by_client_id
  ...
  begin
    $if $$no_op $then
      null;
    $else
      for x in (select client_id from ersh_logger_prefs_by_client_id) loop
        unset_client_level(p_client_id => x.client_id);
      end loop;
    $end
  ...
  procedure append_param(
    p_params in out nocopy ersh_logger.tab_param,
    p_name in varchar2,
    p_val in varchar2
  )
  as
    l_param ersh_logger.rec_param;
  begin
    $if $$no_op $then
      null;
    $else
  ...
  begin
    $if $$no_op $then
      null;
    $else
      ersh_logger.append_param(p_params => p_params, p_name => p_name, p_val => ersh_logger.tochar(p_val => p_val));
    $end
  end append_param;
   ...
   * @param p_line_no
   * @param p_extra
   * @param po_id ID entered into ersh_logger_logs for this record
   */
  procedure ins_ersh_logger_logs(
    p_logger_level in ersh_logger_logs.logger_level%type,
    p_text in varchar2 default null, -- Not using type since want to be able to pass in 32767 characters
    p_scope in ersh_logger_logs.scope%type default null,
    p_call_stack in ersh_logger_logs.call_stack%type default null,
    p_unit_name in ersh_logger_logs.unit_name%type default null,
    p_line_no in ersh_logger_logs.line_no%type default null,
    p_extra in ersh_logger_logs.extra%type default null,
    po_id out nocopy ersh_logger_logs.id%type
    )
  as
    pragma autonomous_transaction;
    ...
    l_extra ersh_logger_logs.extra%type := p_extra;
    l_tmp_clob clob;
  ...
  begin
    ...
    $else
      ...
      select ersh_logger_logs_seq.nextval
      into po_id
      from dual;
  ...
  /**
   * Returns the rec_ersh_logger_logs for given logger_level
   * Used for plugin.
  ...
  function get_plugin_rec(
    p_logger_level in ersh_logger_logs.logger_level%type)
    return ersh_logger.rec_ersh_logger_log
  as
  begin

    if p_logger_level = ersh_logger.g_error then
